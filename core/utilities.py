...
def computeCauchyGreenStrain(F):
    ...
    C = tf.concat([C11, C12, C21, C22], axis=1)
    return C
...
def computeStrainInvariantDerivatives(F,i,secondDerivative=False):
    ...
    if not(secondDerivative):
        dIdF = tf.zeros(F.shape[0],F.shape[1])
        if(i==1):
            # dI1/dF:
            dIdF = 2.0*F
        elif(i==2):
            ...
            dIdF = tf.concat([dIdF11, dIdF12, dIdF21, dIdF22], axis=1)
    ...
    if secondDerivative:
        dIdF = tf.zeros(F.shape[1],F.shape[1])
        if(i==1):
            # d(dI1/dF)/dF:
           dIdF = 2.0 * tf.eye(F.shape[1])
        elif(i==3):
            # d(dI3/dF)/dF:
            J = F11*F22 - F12*F21
            dJdF11 = F22
...
def computeTriangleArea(p1, p2, p3):
    ...
    a = tf.sqrt(vec_a[:, 0:1]**2 + vec_a[:, 1:2]**2)


    b = tf.sqrt(vec_b[:, 0:1]**2 + vec_b[:, 1:2]**2)
