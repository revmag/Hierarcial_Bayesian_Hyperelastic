...
def computeFeatures_torch(I1, I2, I3, Ia, Ib):
    ...
    numFeatures = 28
    x = tf.zeros(I1.shape[0],numFeatures)

    # Some standard terms
    K1 = I1 * tf.pow(I3,-1/3) - 3.0
    K2 = (I1 + I3 - 1) * tf.pow(I3,-2/3) - 3.0
    J = tf.sqrt(I3)
    Iat = Ia * tf.pow(J,-2./3.)
    Ibt = Ib * tf.pow(J,-2./3.)
    ...
    # ####################################################
    # Gent-Thomas:
    i+=1; x[:,i:(i+1)] = tf.log((K2+3.0)/3.0)

    ####################################################
    #Arruda Boyce
    lambda_chain = tf.sqrt(I1 * tf.pow(J,-2./3.)/3.)
    xtilde = lambda_chain / tf.sqrt(tf.tensor([28.]))
    s1 = (tf.sign(0.841-xtilde)+1.)/2.
    s2 = (tf.sign(xtilde-0.841)+1.)/2.
    beta_chain1 = 1.31*tf.tan(1.59*xtilde)+0.91*xtilde
    beta_chain2 = tf.tensor([1.])/(tf.sign(xtilde) - xtilde)
    beta_chain = s1 * beta_chain1 + s2*beta_chain2

    R1 = beta_chain*lambda_chain + tf.sqrt(tf.tensor([28.]))*tf.log(beta_chain/tf.sinh(beta_chain))
    i+=1; x[:,i:(i+1)] = tf.tensor([10.])* (tf.sqrt(tf.tensor([28.]))*R1 - 1.5164)


    ####################################################
    #Ogden
    I1_tilde = J**(-2/3)*I1 + 1e-13
    I1t_0 =tf.tensor([3]) + 1e-13
    J_0 = tf.tensor([1]) + 1e-13
    mu_ogden = 1
    alpha_ogden = 0.65
    W_offset = 1/alpha_ogden * 2. * (0.5**alpha_ogden*(I1t_0  +  tf.sqrt(  (I1t_0-1/(J_0**(2./3.)))**2 - 4*J_0**(2./3.)) - 1/(J_0**(2./3.)) )**alpha_ogden+( 0.5*I1t_0 - 0.5*tf.sqrt(  (I1t_0-1/(J_0**(2./3.)))**2 - 4*J_0**(2./3.))  - 0.5/(J_0**(2./3.)) )**alpha_ogden + J_0**(-alpha_ogden*2./3.) ) * mu_ogden
    W_truth = 1/alpha_ogden * 2. * (0.5**alpha_ogden*(I1_tilde  +  tf.sqrt(  (I1_tilde-1/(J**(2./3.)))**2 - 4*J**(2./3.)) - 1/(J**(2./3.)) )**alpha_ogden+( 0.5*I1_tilde - 0.5*tf.sqrt(  (I1_tilde-1/(J**(2./3.)))**2 - 4*J**(2./3.))  - 0.5/(J**(2./3.)) )**alpha_ogden + J**(-alpha_ogden*2./3.) ) * mu_ogden - W_offset
    i+=1; x[:,i:(i+1)] = W_truth
    ####################################################
    #Ogden2ndterm
    I1_tilde = J**(-2/3)*I1 + 1e-13
    ...
    ####################################################
    #Anisotropy 1 fibre
    i+=1; x[:,i:(i+1)] = tf.pow(Iat - 1.,2.)
    i+=1; x[:,i:(i+1)] = tf.pow(Iat - 1.,3.)
    i+=1; x[:,i:(i+1)] = tf.pow(Iat - 1.,4.)
    ####################################################
    #Anisotropy 2 fibre
    i+=1; x[:,i:(i+1)] = tf.pow(Ibt - 1.,2.)
    i+=1; x[:,i:(i+1)] = tf.pow(Ibt - 1.,3.)
    i+=1; x[:,i:(i+1)] = tf.pow(Ibt - 1.,4.)
    ####################################################
    ...
    k1h = 0.9;
    k2h = 0.8;
    i+=1; x[:,i:(i+1)] = k1h/2./k2h*(tf.exp(k2h*tf.pow(Iat - 1.,2.))-1.);
    i+=1; x[:,i:(i+1)] = k1h/2./k2h*(tf.exp(k2h*tf.pow(Ibt - 1.,2.))-1.);
...
def getNumberOfFeatures():
    ...
    """
    features = computeFeatures_torch(tf.zeros(1,1),tf.zeros(1,1),tf.zeros(1,1),tf.zeros(1,1),tf.zeros(1,1))
