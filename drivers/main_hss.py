...
# here sys.argv[0] is name of script itself, which is main_hss.py
# and sys.argv[1] is material_name, which is ArrudaBoyce
# and sys.argv[2] is noise level,  which is low
# and sys.argv[3] is dynamic data, 
# SYS.ARGV is a list in Python's sys module that contains command-line argument passed to the script
...
if (len(sys.argv)>1):
	...
	if(len(sys.argv)==4):    
		...
		prefix = '/dyn_res='   ##### PREFIX AND 
		fig_prefix = 'Dynamic '   ##### FIG PREFIX HAVE BEEN UPDATED HERE, FOR THE DYNAMIC PART 
	if(len(sys.argv)>4):
...
#----------------------------------------------------------------------------------------
filtered_data_set = []
loadsteps = get_loadsteps(fem_material)     ##WHAT IS LOADSTEP HERE
feature_filter = get_feature_filter(fem_material)

#----------------------------------------------------------------------------------------
#preprocess_data

fem_dir_pp = fem_dir+prefix+fem_res+'_noise='+noise_level+'/'+fem_material;   ## WHAT IS FEM_RES HERE
...
for loadstep in loadsteps:
	...
	# output is in this form - RawData(A1, b1, A2, b2, dof_x, dof_y)
